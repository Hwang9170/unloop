...
export default function RelationGraph({ data }: RelationGraphProps) {
  ...
  const relationshipNodes = [
    { id: 'self', label: '나', x: 50, y: 50, type: 'self' },
    { id: 'main', label: data.main_person, x: 20, y: 30, type: 'main' },
    { id: 'family', label: '가족', x: 80, y: 25, type: 'secondary' },
    { id: 'friends', label: '친구들', x: 75, y: 75, type: 'secondary' },
    { id: 'work', label: '동료', x: 25, y: 80, type: 'secondary' }
  ];
  ...
  return (
    <div className="space-y-6">
      <div>
        <h3 className="text-white text-xl font-medium mb-6">관계 네트워크</h3>
        ...
        <div className="relative h-80 bg-white/5 rounded-xl border border-white/10 mb-6 overflow-hidden">
          ...
          <div className="absolute top-4 right-4 bg-white/10 rounded-lg p-3 backdrop-blur-sm">
            <div className="text-white/80 text-xs mb-1">주요 관계</div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
        ...
        >
          <div className="text-white/80 text-sm mb-2">관계 감정</div>
          <div
        ...
        <motion.div
          ...
          className="bg-white/10 rounded-xl p-4 border border-white/20 text-center"
        >
          <div className="text-white/80 text-sm mb-2">소통 방식</div>
        ...
        <motion.div
          ...
          className="bg-white/10 rounded-xl p-4 border border-white/20 text-center"
        >
          <div className="text-white/80 text-sm mb-2">친밀도</div>
      ...
      <div className="bg-white/5 rounded-xl p-4 border border-white/10">
        <h4 className="text-white text-sm font-medium mb-2">관계 분석</h4>
        <p className="text-white/80 text-sm leading-relaxed">
          오늘 일기에서 가장 중요하게 언급된 인물은 <strong>{data.main_person}</strong>입니다.
          이 관계에서 느끼는 주요 감정은 <strong>{data.emotion}</strong>이며,
          주로 <strong>{data.interaction_type}</strong> 방식으로 소통하고 있습니다.
          현재 친밀도는 {closenessPercentage}%로,
          {closenessPercentage > 80 ? ' 매우 긍정적이고 가까운' :
           closenessPercentage > 60 ? ' 좋은' :
           closenessPercentage > 40 ? ' 보통의' : ' 개선이 필요한'}
          관계로 분석됩니다.
        </p>
      </div>
    </div>
